...
io.on('connection', (socket) => {
	...
	socket.on('createMessage', (message, callback) => {
		...
		if (users.getUser(socket.id) && isRealString(message.text)) {
			io.to(users.getUser(socket.id).room).emit('newMessage', generateMessage(users.getUser(socket.id).name, message.text))
	...
	socket.on('newLocation', (coords) => {
		if (users.getUser(socket.id)) {
			io.to(users.getUser(socket.id).room).emit('newLocationLink', generateLocationMessage(users.getUser(socket.id).name, coords.lat, coords.long))
		}
	})
